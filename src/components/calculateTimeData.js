function calculateTimedata(data) {
  ...
  for (const key in groups) {
    ...
    for (const hour in timeData) {
      ...
      const avgCO2 = +(
        CO2.reduce((sum, value) => sum + value, 0) / CO2.length
      ).toFixed(2);
      const avgH2S = +(
        H2S.reduce((sum, value) => sum + value, 0) / H2S.length
      ).toFixed(2);
      const avgNH3 = +(
        NH3.reduce((sum, value) => sum + value, 0) / NH3.length
      ).toFixed(2);
      const avgHUMT = +(
        HUMT.reduce((sum, value) => sum + value, 0) / HUMT.length
      ).toFixed(2);
      const avgTEMP = +(
        TEMP.reduce((sum, value) => sum + value, 0) / TEMP.length
      ).toFixed(2);
